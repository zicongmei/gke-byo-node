...
IMAGE_NAME=${IMAGE_NAME:-nixos-gke-$(date +%Y%m%d-%H%M)}
GCS_BUCKET=${GCS_BUCKET:-} # Required for image creation

echo "Building NixOS GKE Node GCP image using Docker..."
...
cp "$DIR/configuration_gke.nix" "$BUILD_DIR/"
...
docker run --rm \
  ...
  sh -c "nix-build '<nixpkgs/nixos>' \
    ...
    -I nixos-config=./configuration_gke.nix \
...
if [[ -n "$GCS_BUCKET" ]]; then
  ...
  gcloud storage cp "$IMAGE_PATH" "gs://$GCS_BUCKET/$IMAGE_NAME.tar.gz"
  ...
  echo "gcloud storage cp $IMAGE_PATH gs://YOUR_BUCKET/$IMAGE_NAME.tar.gz"
