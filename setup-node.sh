...
# Decode and place kube-proxy credentials
echo "${KUBE_PROXY_PRIVATE_KEY_BASE64}" | base64 -d > "/var/lib/kube-proxy/kube-proxy.key"
echo "${KUBE_PROXY_CLIENT_CERT_BASE64}" | base64 -d > "/var/lib/kube-proxy/kube-proxy.crt"
...
# Now kube-proxy uses its own specific client certificate and key.
...
kubectl config set-credentials kube-proxy --client-certificate="/var/lib/kube-proxy/kube-proxy.crt" --client-key="/var/lib/kube-proxy/kube-proxy.key" --kubeconfig=/var/lib/kube-proxy/kubeconfig --embed-certs=true >/dev/null
